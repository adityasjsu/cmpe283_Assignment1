...
/*
 * primary procbased controls capabilities
 * See SDM volume 3, section 24.6.2
 */
struct capability_info primary_procbased[22] =
{
...
/*
 * Secondary Processor-Based Controls capabilities
 * See SDM volume 3, section 24.6.2
 */
...
/*
 * Tertiary Processor-Based Controls capabilities
 * See SDM volume 3, section 24.6.2
 */
...
/*
 * Primary VM-Exit Controls capabilities
 * See SDM volume 3, section 24.7.1
 */
struct capability_info primary_vm_exitbased[17] =
{
 ...
 */
struct capability_info vm_entrybased[13] =
{
...
detect_vmx_features(void)
{
	...
	pr_info("Primary Procbased controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(primary_procbased, 22, lo, hi);
	pr_info("\n");
	
	/*checking for the ability to set “Activate Secondary Controls” */
	if(hi & (1 << 31)){
	/*secondary Proc based controls */
	rdmsr(IA32_VMX_PROCBASED_CTLS2, lo, hi);
	pr_info("Secondary Procbased controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(secondary_procbased, 28, lo, hi);
	pr_info("\n");		
	}
	
	else{
		pr_info("Activate Secondary Controls cannnot be set for primary procbased controls");
		
	}
	
		/*checking for the ability to set “Activate Tertiary Controls” */
	if(hi & (1 << 17)){
	/*secondary Proc based controls */
	rdmsr(IA32_VMX_PROCBASED_CTLS3, lo, hi);
	pr_info("Tertiary Procbased controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(tertiary_procbased, 28, lo, hi);
	pr_info("\n");		
	}
	
	else{
		pr_info("Activate Tertiary Controls cannnot be set for primary procbased controls");
		
	}
	
		/* Primary VM-Exit Controls*/
	rdmsr(IA32_VMX_EXIT_CTLS, lo, hi);
	pr_info("Primary VM-Exit Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(primary_vm_exitbased, 17, lo, hi);
	pr_info("\n");

		/* Primary VM-Entry Controls*/
	rdmsr(IA32_VMX_ENTRY_CTLS, lo, hi);
	pr_info("Primary VM-Exit Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(vm_entrybased, 13, lo, hi);
	pr_info("\n");	
}
