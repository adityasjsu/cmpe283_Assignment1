...
static int __vmx_handle_exit(struct kvm_vcpu *vcpu, fastpath_t exit_fastpath)
{
	...
	// local uint64_t variables for record the beginning and the ending of processor's time stamp
	uint64_t begin_time, end_time;
	// local int variable to store the return status of vmx handler exit function
	int vmx_exit_handler_status;
	...
	arch_atomic_inc(&total_no_exits);

	// increase by 1 for current type-specified exit
	if (exit_reason.basic < 70) {
		arch_atomic_inc(&type_total_no_exits[(int)exit_reason.basic]);
	}
	// record the beginning of cpu's time stamp
	begin_time = rdtsc();

	// call  exit handler to handle the exit
	vmx_exit_handler_status = kvm_vmx_exit_handlers[exit_handler_index](vcpu);

	// record the end of cpu's time stamp
	end_time = rdtsc();

	// compute the current time stamp gap and add it to the cpu cycle time of total exits
	arch_atomic64_add((end_time - begin_time), &total_no_cpu_cycles);

	// compute the current time stamp gap and add it to the cpu cycle time of type-specified exit
	if (exit_reason.basic < 70) {
		arch_atomic64_add((end_time - begin_time), &type_total_no_cpu_cycles[(int)exit_reason.basic]);
	}

	return vmx_exit_handler_status;
...
extern atomic_t total_no_exits;
//total number of cpu cycles on exits
extern atomic64_t total_no_cpu_cycles;

static int vmx_handle_exit(struct kvm_vcpu *vcpu, fastpath_t exit_fastpath)
{

	// beginning and the ending of processor's time
	uint64_t begin_time, end_time;

	int ret = __vmx_handle_exit(vcpu, exit_fastpath);
	arch_atomic_inc(&total_no_exits);

	//  beginning of cpu's time
	begin_time = rdtsc();

	//  the ending of cpu's time
	end_time = rdtsc();
	// time gap added to the total cpu cycles
	arch_atomic64_add((end_time - begin_time), &total_no_cpu_cycles);
